...
_get_kernel_param_value() {
    ...
    for _param in "${_params[@]}"; do
        _key="${_param%=*}"
        _value="${_param#*=}"
        if [[ "${_target_param}" == "${_key}" && "${_key}" != "${_value}" ]];\
        then
            printf "%s" "${_value}"
            break
        fi
...
# key_command is always evaluated at the last suitable moment in order
...
printf "config-drive partition dev path:%s\nkey script:%s\nkey command:%s\n"\
    "${config_drive_dev_path:-auto}" "${key_script}" "${key_command}"
...
elif [[ -n "${config_drive_part_num}" && -z "${config_drive_dev_path}" ]];\
then
    config_drive_dev_path="${config_drive_common}${config_drive_part_num}"
fi
printf "INFO: preview of generated config drive metadata:\n"
...
if [[ "/dev/${root_partition}" == "${config_drive_dev_path}" ]]; then
    ...
    printf "WARNING: Real config drive was not detected, exiting!\n"
    exit 0
fi
if _is_luks "${config_drive_dev_path}"; then
    ...
    _dry_run "/usr/lib/systemd/systemd-cryptsetup" "attach" "config-2" \
            "${config_drive_dev_path}" <(${key_command}) "luks"
